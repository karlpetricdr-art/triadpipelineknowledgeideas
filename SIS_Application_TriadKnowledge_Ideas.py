...
# =============================================================================
# 5. CEREBRAS TRIAD SYNERGY ENGINE (0.85 -> 0.65 -> 0.4)
# =============================================================================

if st.button("🚀 EXECUTE CEREBRAS TRIAD PIPELINE", use_container_width=True):
    if not cerebras_api_key:
        st.error("❌ Cerebras API key is required for the Triad System.")
    elif not user_query:
        st.warning("⚠️ Initial Research Inquiry is required.")
    else:
        try:
            # Initialize Cerebras Client
            client = OpenAI(api_key=cerebras_api_key, base_url="https://api.cerebras.ai/v1")
            
            # Fetch Metadata (ORCID/Scholar)
            biblio = fetch_author_bibliographies(target_authors) if target_authors else ""

            # --- PHASE 1: FOUNDATION (IMA Logic) | Temp: 0.85 ---
            with st.spinner('PHASE 1: Cerebras establishing Research Foundation (Temp 0.85)...'):
                p1_sys = f"""You are SIS Phase 1: Structural Architect. 
                Focus: Integrated Metamodel Architecture (IMA). 
                Context: {sel_sciences}, {sel_paradigms}. 
                Authors: {biblio}.
                Goal: Map the 'Identity', 'Mission', and 'Problem' space. High creativity/exploration."""
                ...
                phase1_output = p1_res.choices[0].message.content

            # --- PHASE 2: INNOVATION (MA Logic) | Temp: 0.65 ---
            with st.spinner('PHASE 2: Cerebras generating Innovative Ideas (Temp 0.65)...'):
                p2_sys = f"""You are SIS Phase 2: Creative Catalyst. 
                Focus: Mental Approaches (MA) - Dialectics, Perspective Shifting, and Induction.
                Task: Take Phase 1 foundation and generate radical 'Useful Innovative Ideas'."""
                
                p2_prompt = f"RESEARCH FOUNDATION:\n{phase1_output}\n\nINNOVATION GOAL:\n{idea_query}"
                ...
                phase2_output = p2_res.choices[0].message.content

            # --- PHASE 3: SYNTHESIS & GRAPH | Temp: 0.4 ---
            with st.spinner('PHASE 3: Cerebras finalizing Logic & Semantic Mapping (Temp 0.4)...'):
                p3_sys = f"""You are SIS Phase 3: Systems Integrator. 
                Focus: Logical Rigor, Taxonomy, and Graph Visualization.
                Task: Consolidate previous phases into a final report.
                CRITICAL: You must end with '### SEMANTIC_GRAPH_JSON' and a valid JSON object.
                JSON Schema: {{"nodes": [...], "edges": [...]}}
                Node Shapes: IMA=rectangle, MA=diamond."""
                
                p3_prompt = f"PHASE 1 (FOUNDATION):\n{phase1_output}\n\nPHASE 2 (IDEAS):\n{phase2_output}\n\nFINAL SYNTHESIS REQUEST: Integrate and map."
                ...
                phase3_output = p3_res.choices[0].message.content

            # --- RENDERING THE TRIAD RESULTS ---
            full_report = f"# 🎓 SIS Triad Synthesis Report\n\n## 🏛️ Phase 1: Foundation\n{phase1_output}\n\n## 💡 Phase 2: Innovation\n{phase2_output}\n\n## 🧬 Phase 3: Final Synthesis\n{phase3_output}"
            
            # Split for JSON parsing
            ...
            if len(parts) > 1:
                try:
                    g_json = json.loads(re.search(r'\{.*\}', parts[1], re.DOTALL).group())
                    st.subheader("🕸️ TRIAD SEMANTIC NETWORK")
                    elements = []
                    for n in g_json.get("nodes", []):
                        elements.append({"data": {"id": n["id"], "label": n["label"], "color": n.get("color", "#2a9d8f"), "size": 100, "shape": n.get("shape", "rectangle"), "z_index": 1}})
                    for e in g_json.get("edges", []):
                        elements.append({"data": {"source": e["source"], "target": e["target"], "rel_type": e.get("rel_type", "leads_to")}})
                    render_cytoscape_network(elements, "viz_triad_950")
                except: st.warning("⚠️ Graph JSON parsing failed in Phase 3.")

        except Exception as e:
            st.error(f"❌ Triad Pipeline Error: {e}")
# =============================================================================
# 6. FOOTER & METRICS
# =============================================================================
st.divider()
