...
VERSION_CODE = "v33.8.0-CEREBRAS-TRIAD-PREMIUM-FULL"
...
HUMAN_THINKING_METAMODEL = {
    "nodes": {
        "Human mental concentration": {"color": "#ADB5BD", "shape": "rectangle", "desc": "Foundational cognitive focus."},
        "Identity": {"color": "#C6EFCE", "shape": "rectangle", "desc": "Subjective core and ethical parameters."},
        "Autobiographical memory": {"color": "#C6EFCE", "shape": "rectangle", "desc": "Historical cycles influencing current logic."},
        "Mission": {"color": "#92D050", "shape": "rectangle", "desc": "Existential imperative driving inquiry."},
        "Vision": {"color": "#FFFF00", "shape": "rectangle", "desc": "Simulation of desired future outcomes."},
        "Goal": {"color": "#00B0F0", "shape": "rectangle", "desc": "Quantifiable milestones."},
        "Problem": {"color": "#F2DCDB", "shape": "rectangle", "desc": "Obstruction and gaps in hierarchies."},
        "Ethics/moral": {"color": "#FFC000", "shape": "rectangle", "desc": "Value filtering system."},
        "Hierarchy of interests": {"color": "#F8CBAD", "shape": "rectangle", "desc": "Ordering of systemic needs."},
        "Rule": {"color": "#F2F2F2", "shape": "rectangle", "desc": "Structural and logical constraints."},
        "Decision-making": {"color": "#FFFF99", "shape": "rectangle", "desc": "Pathway selection algorithms."},
        "Problem solving": {"color": "#D9D9D9", "shape": "rectangle", "desc": "Execution of corrective actions."},
        "Conflict situation": {"color": "#00FF00", "shape": "rectangle", "desc": "Goal clashes and tensions."},
        "Knowledge": {"color": "#DDEBF7", "shape": "rectangle", "desc": "Internalized facts and models."},
        "Tool": {"color": "#00B050", "shape": "rectangle", "desc": "External instruments for domain interaction."},
        "Experience": {"color": "#00B050", "shape": "rectangle", "desc": "Wisdom through direct application."},
        "Classification": {"color": "#CCC0DA", "shape": "rectangle", "desc": "Taxonomic cognitive reduction."},
        "Psychological aspect": {"color": "#F8CBAD", "shape": "rectangle", "desc": "Internal mental states."},
        "Sociological aspect": {"color": "#00FFFF", "shape": "rectangle", "desc": "Collective social impacts."}
    }
}
...
HIERARCHOLOGY_ONTOLOGY = {
    "core_definitions": {
        ...
        "Hierarchography": "Descriptive visual outlining using workflows and diagrammatic logic.",
    ...
    "operational_logic": {
        "Internal Processes": "Inductive (specific to general).",
        "External Functioning": "Deductive & Dialectical (general to specific)."
    },
    "hierarchography_tools": ["Workflow Mapping", "Tree Maps", "Oligographs", "UML", "Mind Mapping", "Organigrams"]
}

IDEATION_TECHNIQUES = {
    "SCAMPER": "Substitute, Combine, Adapt, Modify, Put to another use, Eliminate, Reverse.",
    "First Principles": "Break down complex problems into basic elements and reassemble from scratch.",
...
# =============================================================================
# 3. KNOWLEDGE BASE (EXHAUSTIVE SCIENCE FIELDS & PARADIGMS)
# =============================================================================
...
KNOWLEDGE_BASE = {
    ...
    "Scientific paradigms": {
        "Empiricism": "Focus on sensory experience and sensory data.",
        "Rationalism": "Reliance on deductive logic and a priori reasoning.",
        "Constructivism": "Knowledge as a social and cognitive build.",
        "Positivism": "Strict adherence to verifiable facts without speculation.",
...
with st.sidebar:
    ...
    with st.expander("🧠 MENTAL APPROACHES (MA)", expanded=False):
...
st.markdown(f"**Pure Cerebras Sequential Pipeline** | Current Operating Date: **{SYSTEM_DATE}**")
...
if st.button("🚀 EXECUTE PURE CEREBRAS TRIAD PIPELINE", use_container_width=True):
    if not cerebras_api_key:
        ...
        try:
            ...
            paradigms_str = json.dumps(KNOWLEDGE_BASE["Scientific paradigms"])

            # --- PHASE 1: CEREBRAS (VISIONARY - 0.85) ---
            with st.spinner('PHASE 1: Cerebras razvija vizionarsko podlago (0.85)...'):
                p1_template = """
                You are a Hierarchology Visionary (Phase 1). 
                STRICT IMA Architecture: [IMA] | HIERARCHOLOGY Basis: [BASIS] | SCIENCES: [SCIENCES] | PARADIGMS: [PARADIGMS]
                TASK: speculatively identify hidden hierarchies across MICRO, MESO and MACRO.
                Integrate hard science analogies (Physics, Biology) to break the 'Scientific Cage'.
                Context from file: [FILE]
                """
                p1_c = p1_template.replace("[IMA]", ima_str).replace("[BASIS]", h_ont).replace("[SCIENCES]", str(sel_sciences)).replace("[PARADIGMS]", paradigms_str).replace("[FILE]", file_content)
            ...
            with st.spinner('PHASE 2: Cerebras razvija inovativne rešitve (0.65)...'):
                p2_template = """
                You are the SIS Innovation Engine (Phase 2). 
                MA FOCUS: [MA] | TOOLBOX: [TECH] | SCIENCES: [SCIENCES]
                TASK: Generate 5 radical ideas acting on Micro, Meso, or Macro levels.
                Pick ONE specific technique from [TECH] for each idea (SCAMPER, TRIZ, etc.).
                Use Analogical Reasoning from physical laws to social solutions.
                """
            ...
            # --- PHASE 3: CEREBRAS (FINAL SYNERGY & STABLE JSON - 0.45) ---
            with st.spinner('PHASE 3: Cerebras izvaja finalno sintezo in Hierarhografijo (0.45)...'):
                p3_prompt = """
                Produce a 'Perfect 10' FINAL SYNERGY REPORT with the 'Heartbeat of Truth'.
                
                STRICT RULES:
                1. DO NOT include color codes, hex codes (#...), or technical parameters in the narrative.
                2. Use EXACT node labels (e.g. 'Entropy Shield') for linking.
                3. HIERARCHOGRAPHY MESH: Connect every Innovation (star) to Micro (ellipse) AND Macro (octagon) nodes.
                4. RELATIONS: micro_to_meso, meso_to_macro, outcome_of, leads_to, BT, NT, AS, EQ, RT.
            ...
            # --- ROBUST PROCESSING & FUZZY LINKING ---
            ...
            if graph_json_str:
                try:
                    ...
                    # Sort nodes by length (longest first) to avoid partial replacement bugs
                    ...
                    for n in nodes:
                        ...
                        # Google Linker (Fuzzy Case-Insensitive Regex)
                        if len(lbl) > 2:
            ...
            st.subheader("📊 FINAL TRIAD VERIFIED SYNERGY")
